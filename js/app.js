...
function init(){
  initShell(); initExperience(); initProfile(); initStateEngine(); initEventLab(); initIntelligence(); initPerformanceLog(); initResearchNof1(); initHealth(); initHandoff(); bindResearchUI(); prospectiveController=initProspective(()=>currentState); pallasController=initPallas(()=>currentState);
...
function initShell(){
  ...
  const setGuideMode=mode=>{const m=mode==='reviewer'?'reviewer':'athlete';$$('[data-guide-mode]').forEach(btn=>{const active=btn.dataset.guideMode===m;btn.classList.toggle('active',active);btn.setAttribute('aria-selected',String(active))});$$('[data-guide-pane]').forEach(p=>p.classList.toggle('active',p.dataset.guidePane===m));};
  $('#guideToggle')?.addEventListener('click',()=>{setGuideMode('athlete');setGuide(!$('#guideDrawer')?.classList.contains('open'))});
  $('#startGuide')?.addEventListener('click',()=>{setGuideMode('athlete');setGuide(true)});
  $$('[data-guide-mode]').forEach(btn=>btn.addEventListener('click',()=>setGuideMode(btn.dataset.guideMode)));
  $$('[data-open-reviewer]').forEach(btn=>btn.addEventListener('click',()=>{setGuideMode('reviewer');setGuide(true)}));
